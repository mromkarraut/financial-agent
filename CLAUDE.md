...
Telegram message
  ...
  → _build_calls(text, chat_id)  [regex routing]
      options keywords present?  → OptionsResearchAgent only
      otherwise                  → StockResearchAgent + FundamentalsAgent (parallel)
...
The local model (Qwen2.5-0.5B via LM Studio) frequently outputs `?????` garbage. Both `stock_research.py` and `fundamentals_agent.py` guard against this:
...
### OptionsResearchAgent internals

`agents/options_research_agent.py` is the primary agent. No LLM calls — only yfinance + Black-Scholes.
...
| `LLM_MODEL` | `qwen2.5-0.5b-instruct` | Must match model loaded in LM Studio |
